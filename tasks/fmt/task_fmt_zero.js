...
/***/ 9:
/* exports provided: TaskFmtZero */
/* all exports used */
/*!****************************************!*\
  !*** ./src/task_scheme/task_scheme.js ***!
  \****************************************/
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony export (immutable) */ __webpack_exports__["TaskFmtZero"] = TaskFmtZero;
/**
...
/**
 * создаем класс TaskFmtZero, конструктор может быть пустым, инициализация происходит позже
 ...
 * @constructor
 */
function TaskFmtZero(settings) {
    this.settings = settings;
    this.fmt = qwerty00006.magic();
}
 ...
 */
TaskFmtZero.prototype.id = function () {
    return "TaskFmtZero";
};

TaskFmtZero.prototype.initCounter=0;
 ...
 */
TaskFmtZero.prototype.initialize = function (domNode, kioapi, preferredWidth) {
    //сохраняем данные для будущего использования
    this.kioapi = kioapi;
    TaskFmtZero.prototype.initCounter++;
    $(domNode).attr('id', 'fmtId'+TaskFmtZero.prototype.initCounter);
    this.fmt.init(kioapi, 'fmtId'+TaskFmtZero.prototype.initCounter, preferredWidth, JSON.parse('{"input":"мама", "alphabet":"мчшщжыиая", "outalphabet":"78", "outfsm":false, "height":500, "vertexsize":25}'));
 ...
 * для сравнения:
 */
TaskFmtZero.prototype.parameters = function () {
    return [
      {
        name: "info1",
        title: "Количество проверяемых автоматом правильных слов без ошибок",
      ...
      {
        name: "info2",
        title: "Количество проверяемых автоматом правильных слов с ошибками",
        // view: function (v) { //отображаем вещественное число как две точки после запятой, и потом процент
        //   return  "416";
        // },
        normalize: function(v) { //перед сравнением параметра значение превращается в 0
      ...
      {
        name: "info3",
        title: "Количество проверяемых автоматом неправильных слов",
        // view: function (v) { //отображаем вещественное число как две точки после запятой, и потом процент
        //   return  "5761";
        // },
        normalize: function(v) { //перед сравнением параметра значение превращается в 0
      ...
      {
        name: "correct", //название параметра
        title: "Число баллов по первому критерию", //отображение названия для пользователя
 ...
 */
TaskFmtZero.prototype.solution = function () {
    return this.fmt.solution();
};
 ...
 */
TaskFmtZero.prototype.loadSolution = function (solution) {
    if (!solution) return;
    this.fmt.load(solution);
};
        ...
        var qwerty00006 = (function () {
            ...
            function Fsm() {
                ...
                this.gui = {
                    ...
                    vertexColorRed: "#ff4444",
                    vertexColorGreen: "#00C851",
                    vertexColorYellow: "#ffbb33",

                    vertexColorCurrent: "#000",

                    vertexColorDark: "#1b6d85",

                    vertexStrokeColor: "#777",
          ...
          Fsm.prototype.loadExamples = function(templates, dest){
            for (var i = 0; i < templates.length; i++) {
              ...
              for (var j = 0; j < temp[0].length; j++) {
                ...
                for (var k = 0; k < temp[1].length; k++) {
                  var temp1 = temp[1][k];
                  for (var l = 0; l < temp[2].length; l++) {
                    var temp2 = temp[2][l];
                    for (var m = 0; m < temp[3].length; m++) {
                      dest.push(temp0+temp1+temp2+temp[3][m]);
                    }
                  }
                }
              }
            ...
            Fsm.prototype.init = function (kio, divId, taskWidth, config) {
                this.divId = divId;
                this.config = config;
              ...
              this.loadExamples(this.testgt, this.testg);
              this.loadExamples(this.testrt, this.testr);
              this.loadExamples(this.testyt, this.testy);
              console.log(new Date().getTime()-now);
                ...
                this.baseG = new Base(this, this.gui, this.gui.vertexMargin, this.gui.height + this.gui.vertexMargin, "G");
                this.baseY = new Base(this, this.gui, this.gui.vertexMargin * 2 + this.gui.vertexSize, this.gui.height + this.gui.vertexMargin, "Y");
                this.baseR = new Base(this, this.gui, this.gui.vertexMargin * 3 + this.gui.vertexSize*2, this.gui.height + this.gui.vertexMargin, "R");
                ...
                this.firstState = new State(this.gui.width/10, this.gui.height/2, "G", this.gui, this, null);
              ...
              this.load(JSON.parse('{"states":[{"id":"v0","x":0.2552667826282361,"y":0.7598315877007751,"col":"G","label":"S0","first":true},{"id":"v1","x":0.4608443173559003,"y":0.1726412622162793,"col":"G","label":"S1","first":false},{"id":"v2","x":0.7037315126252711,"y":0.793499586500517,"col":"G","label":"S2","first":false},{"id":"v3","x":0.18874969085500298,"y":0.18992709464470306,"col":"R","label":"S3","first":false},{"id":"v4","x":0.5071347231103821,"y":0.4437262953648579,"col":"Y","label":"S4","first":false},{"id":"v5","x":0.7350543478260869,"y":0.247,"col":"G","label":"S5","first":false}],"transitions":[{"from":"v0","to":"v1","label":"ш"},{"from":"v0","to":"v2","label":"ы"},{"from":"v0","to":"v2","label":"и"},{"from":"v1","to":"v3","label":"ы"},{"from":"v2","to":"v4","label":"ы"},{"from":"v2","to":"v4","label":"и"},{"from":"v1","to":"v4","label":"ш"},{"from":"v1","to":"v5","label":"и"},{"from":"v2","to":"v5","label":"ш"}],"input":"мама"}'));
            };


          Fsm.prototype.submitResult=function(){
            if(this.loading){
              return;
            }
            ...
            for (var i = 0; i < this.testg.length; i++) {
              var test = this.testg[i];
              if(this.calc(test)=="G"){
                greens++;
              }
            }
            ...
            for (var i = 0; i < this.testr.length; i++) {
              var test = this.testr[i];
              if(this.calc(test)=="R"){
                reds++;
              }
            }
            ...
            for (var i = 0; i < this.testy.length; i++) {
              var test = this.testy[i];
              if(this.calc(test)=="Y"){
                yellows++;
              }
            }
            ...
            this.kio.submitResult({
              correct: Math.round((greens/this.testg.length)*(reds/this.testr.length)*(yellows/this.testy.length)*100*10000000)/10000000,
              elements:  this.states.length,
              info1: greens,
              info2: reds,
              info3: yellows
            });
          ...
          Fsm.prototype.calc = function(input){
            var curState = this.firstState;
            while(input.length>0){
              var transitions = this.getTransitions(curState);
              var done = false;
              for (var i = 0; i < transitions.length; i++) {
                var transition = transitions[i];
                if (transition.label == input[0]) {
                  done=true;
                  curState = transition.v2;
                }
              }
              if(!done){
                return false;
            ...
            Fsm.prototype.load = function (solution) {
              ...
              Fsm.prototype.loading=true;
                ...
                for (var i = 0; i < solution.states.length; i++) {
                    ...
                    var vertex = new State(v.x * this.gui.width, v.y * this.gui.height, v.col, this.gui, this, null);
                    if(v.first){
            ...
            Fsm.prototype.solution = function () {
                ...
                for (var i = 0; i < this.states.length; i++) {
                    ...
                    result.states.push({id: state.id, x: state.view.x / this.gui.width, y: state.view.y / this.gui.height,
                        col: state.col, label: state.label, first: state==this.firstState});
                }
            ...
            function Base(graph, gui, x, y, col) {
                this.gui = gui;
                this.x = x;
                this.y = y;
                this.graph = graph;
                ...
                var base = this;
                this.vertexBase = new State(x, y, this.col, gui, base.graph, base);
            ...
            Base.prototype.recoverVertex = function () {
                this.vertexBase = new State(this.x, this.y, this.col, this.gui, this.graph, this);
                this.gui.stage.addChild(this.vertexBase.view);
                this.vertexBase.onBase = true;
            };

            function State(x, y, col, gui, graph, base) {
                this.gui = gui;
                ...
                if(this.col=="R"){
                    this.circle.graphics.beginFill(gui.vertexColorRed);
                }else if(this.col=="Y"){
                    this.circle.graphics.beginFill(gui.vertexColorYellow);
                }else{
                    this.circle.graphics.beginFill(gui.vertexColorGreen);
                }
                ...
                view.on("pressup", function () {
                    ...
                    if(!vertex.graph.config.outfsm && vertex.doubleselected && view.x==view.oldX && view.y==view.oldY) {
                        if(vertex.col=="Y"){
                            vertex.col = "R";
                        }else if(vertex.col=="R"){
                            vertex.col = "G";
                        }else{
                            vertex.col = "Y";
                        }
                      vertex.graph.submitResult();
                ...
                mover.on("pressup", function () {
                    ...
                    if(another!=null){
                        ...
                        if(vertex.graph.canAddEdge(vertex)) {
                            var another = new State(mover.x, mover.y, "G", vertex.gui, vertex.graph, null);
            ...
            State.prototype.update = function(current){
                ...
                this.circle = new createjs.Shape();
                if(this.col=="R"){
                    this.circle.graphics.beginFill(this.gui.vertexColorRed);
                }else if(this.col=="Y"){
                    this.circle.graphics.beginFill(this.gui.vertexColorYellow);
                }else{
                    this.circle.graphics.beginFill(this.gui.vertexColorGreen);
                }
...
/******/ });
//# sourceMappingURL=task_scheme.js.map